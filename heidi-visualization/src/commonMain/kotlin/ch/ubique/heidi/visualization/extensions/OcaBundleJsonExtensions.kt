...
import ch.ubique.heidi.visualization.oca.model.computeBundleDigest
import ch.ubique.heidi.visualization.oca.model.computeCesrEncodedDigest

fun OcaBundleJson.verifyIntegrity() : Boolean {
	val captureBaseDigest = computeCesrEncodedDigest(this.captureBase, digest = this.captureBase.digest.first().toString())
	if(captureBaseDigest != this.captureBase.digest) {
		return false
	}
	return this.overlays.all { overlay ->
		overlay.captureBase == captureBaseDigest && computeCesrEncodedDigest(overlay) == overlay.digest
	}
}
fun OcaBundleJson.calculateSaids() : OcaBundleJson {
	val captureBaseDigest = computeCesrEncodedDigest(this.captureBase)
...
fun OcaBundleJson.getBundleHash(digest: String = "I") : String {
	return computeBundleDigest(this, digest)
}
